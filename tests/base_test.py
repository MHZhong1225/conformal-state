        ...
        for method in args['methods'].keys():
            ...
            for lr in lrs:
                if asymmetric:
                    ...
                    if scores_arr.dtype == "object":
                        stacked_scores = np.stack(scores_arr.to_list())
                    else:
                ...
                if real_data:
                    F = np.stack([np.asarray(x, dtype=float) for x in data["forecasts"].to_list()])
                    F_interp = (
                        pd.DataFrame(F, index=data.index)
                        .interpolate(limit_direction="both")
                        .to_numpy()
                    )
                    n = min(len(q), F_interp.shape[0])
                    sets = [set_function(F_interp[i], q[i]) for i in range(n)]
                    sets = [
                        np.array([
                            np.minimum(sets[j][0], sets[j][1] - minsize),
                            np.maximum(sets[j][1], sets[j][0] + minsize),
                        ])
